...
class FormulaRecognizer:
    ...
    def setup_ui(self):
        ...
        self.original_canvas.create_text(
            200, 125, text="选择图片或按 Ctrl+V 粘贴", fill="gray", font=("Arial", 12)
        )
        ...
        self.result_vis_canvas.create_text(
            200, 125, text="识别结果将在此显示", fill="gray", font=("Arial", 12)
        )
        ...
        # API端口选择
        tk.Label(config_frame, text="API端口:").grid(
        ...
        tk.Radiobutton(
            endpoint_frame, text="轻量版", variable=self.api_endpoint_var, value="turbo"
        ).pack(side=tk.LEFT)
        ...
        self.select_btn = tk.Button(
            button_frame, text="选择图片", command=self.select_image
        )
        self.select_btn.pack(side=tk.LEFT, fill=tk.X, expand=True, padx=(0, 2))

        # 截图按钮（简化版）
        self.screenshot_btn = tk.Button(
            button_frame, text="截图识别", command=self.take_screenshot
        )
        ...
        self.result_text = scrolledtext.ScrolledText(
            result_frame, wrap=tk.WORD, height=20, font=("Consolas", 12)
        )
    ...
    def take_screenshot(self):
        ...
        # 提示用户按PrintScreen键
        messagebox.showinfo(
            "截图操作", "请按键盘上的 PrintScreen 键截取屏幕，然后点击确定。"
        )
    ...
    def display_original_image(self, image_path):
        ...
        try:
            ...
            self.original_canvas.delete("all")
            self.original_canvas.create_text(
                200, 150, text=f"图片加载失败: {str(e)}", fill="red", font=("Arial", 12)
            )
    ...
    def display_result_visualization(self, latex_formula):
        ...
        try:
            ...
            self.result_vis_canvas.delete("all")
            self.result_vis_canvas.create_text(
                200, 150, text=f"显示失败: {str(e)}", fill="red", font=("Arial", 12)
            )
